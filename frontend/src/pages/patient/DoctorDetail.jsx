        ...
        <div className="flex flex-col sm:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h1 className="text-2xl font-bold text-white">Dr. {userInfo?.name}</h1>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4 text-sm text-slate-400">
              ...
              <div className="space-y-3">
                {doc.qualifications.map((q, i) => (
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-200 text-sm">{q.degree}</p>
                ...
                {reviews.map((r) => (
                  <div key={r._id} className="border-b border-white/5 last:border-0 pb-4 last:pb-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-200">{r.patientId?.userId?.name}</p>
                        <StarRating rating={r.rating} size="sm" />
                      </div>
                      <span className="ml-auto text-xs text-slate-400">{format(new Date(r.createdAt), 'MMM dd, yyyy')}</span>
                    </div>
                    {r.comment && <p className="text-sm text-slate-400 ml-11">{r.comment}</p>}
                    {r.doctorReply && (
              ...
              <div className="space-y-2">
                {doc.availability.filter((a) => a.isAvailable).map((a, i) => (
                  <div key={i} className="flex items-center justify-between text-sm py-2 border-b border-white/5 last:border-0">
                    <span className="font-medium text-slate-200">{a.day}</span>
                    <span className="text-slate-400">{a.startTime} – {a.endTime}</span>
                  </div>
                ))}
              </div>
            )}
