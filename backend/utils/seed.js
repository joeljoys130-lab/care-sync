...
const seed = async () => {
  ...
  // Admin
  await User.create({
    name: 'Admin User', email: 'admin@caresync.com',
  ...
  const doctorUsers = await User.create([
    { name: 'Sarah Johnson',   email: 'sarah@caresync.com',   password: await hash('doctor123'), role: 'doctor', isVerified: true, isActive: true },
    { name: 'Michael Chen',    email: 'michael@caresync.com',  password: await hash('doctor123'), role: 'doctor', isVerified: true, isActive: true },
    { name: 'Emily Rodriguez', email: 'emily@caresync.com',    password: await hash('doctor123'), role: 'doctor', isVerified: true, isActive: true },
    { name: 'James Wilson',    email: 'james@caresync.com',    password: await hash('doctor123'), role: 'doctor', isVerified: true, isActive: true },
    { name: 'Priya Patel',     email: 'priya@caresync.com',    password: await hash('doctor123'), role: 'doctor', isVerified: true, isActive: true },
  ]);
  ...
  const doctorProfiles = [
    ...
    { specialization: 'Pediatrics',   experience: 10, fees: 120, bio: "Compassionate pediatrician dedicated to children's health and wellness.",              city: 'Boston',        hospital: "Children's Care Center",   rating: 4.9, totalReviews: 302, isApproved: true },
  ];
  ...
  const patientUsers = await User.create([
    { name: 'Alice Thompson', email: 'alice@example.com', password: await hash('patient123'), role: 'patient', isVerified: true, isActive: true },
    { name: 'Bob Martin',     email: 'bob@example.com',   password: await hash('patient123'), role: 'patient', isVerified: true, isActive: true },
    { name: 'Carol Davis',    email: 'carol@example.com', password: await hash('patient123'), role: 'patient', isVerified: true, isActive: true },
  ]);
